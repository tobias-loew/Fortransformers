...
public:   
  ...
  template<int N> struct tvresult {
    ...
    constexpr bool isUnitStride(int)    const {
...
_bz_ArrayExprFullReduce(T_expr const& expr, T_reduction reduction);

template<typename T_expr, typename T_reduction>
_bz_typename T_reduction::T_resulttype
_bz_reduceWithIndexTraversal(T_expr const& expr, T_reduction reduction);

template<typename T_expr, typename T_reduction>
_bz_typename T_reduction::T_resulttype
_bz_reduceWithIndexVectorTraversal(T_expr const& expr, T_reduction reduction);
...
\
/* optimized version for TinyVecotr */\
\
template<typename P_numtype, int N_length>                                              \
 _bz_inline_et								\
_bz_typename reduction<P_numtype>::T_resulttype    \
 fn(const BZ_BLITZ_SCOPE(TinyVector)<P_numtype, N_length>& expr)				\
{                                                                       \
  return _bz_ArrayExprFullReduce_tv					\
    (expr,		\
     reduction<_bz_typename P_numtype>());			\
}                                                                       \
\
/* optimized version for TinyBasedVecotr */\
\
template<typename P_numtype, int N_length, int N_base>                                              \
 _bz_inline_et								\
_bz_typename reduction<P_numtype>::T_resulttype    \
 fn(const BZ_BLITZ_SCOPE(TinyBasedVector)<P_numtype, N_length, N_base>& expr)				\
{                                                                       \
  return _bz_ArrayExprFullReduce_tbv					\
    (expr,		\
     reduction<_bz_typename P_numtype>());			\
}                                                                       \
\
...
\
template<typename P_numtype, int N_rank, int N_base>                                              \
 _bz_inline_et								\
_bz_typename reduction<P_numtype>::T_resulttype    \
 fn(const BZ_BLITZ_SCOPE(Array)<P_numtype, N_rank, BZ_BLITZ_SCOPE(array_policy::StaticFortranArrayPolicy<N_base>)>& expr)				\
{                                                                       \
  return _bz_ArrayExprFullReduce_StaticFortranArrayPolicy					\
    (expr,		\
     reduction<_bz_typename P_numtype>());			\
}                                                                       \
