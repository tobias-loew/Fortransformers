...
struct asExpr<Array<T,N, Policy> > {
  //typedef FastArrayIterator<T,N> T_expr;
  typedef _bz_ArrayExpr<FastArrayIterator<T,N, Policy> > T_expr;
  static T_expr getExpr(const Array<T,N, Policy>& x);
};
...
struct asExpr<TinyBasedVector<T, N, B> > {
    typedef _bz_ArrayExpr<FastTBV2Iterator<T, N, B> > T_expr;
    static T_expr getExpr(const TinyBasedVector<T, N, B>& x);
};
...
struct asExpr<TinyBasedMatrix<T, Nr, Nc, Br, Bc, CM> > {
    typedef _bz_ArrayExpr<FastTBM2Iterator<T, Nr, Nc, Br, Bc, CM> > T_expr;
    static T_expr getExpr(const TinyBasedMatrix<T, Nr, Nc, Br, Bc, CM>& x);
};
...
template<typename T, int N, typename Policy>
struct asET<Array<T,N, Policy> > {
  typedef ETBase<typename asExpr<Array<T,N, Policy> >::T_expr> T_wrapped;
  typedef typename asExpr<Array<T,N, Policy> >::T_expr T_expr;
};
template<typename T, int N>
...
template<typename T, int N, int B>
struct asET<TinyBasedVector<T, N, B> > {
    typedef ETBase<typename asExpr<TinyBasedVector<T, N, B> >::T_expr> T_wrapped;
    typedef typename asExpr<TinyBasedVector<T, N, B> >::T_expr T_expr;
};
template<typename T, int Nr, int Nc>
...
template<typename T, int Nr, int Nc, int Br, int Bc, bool CM>
struct asET<TinyBasedMatrix<T, Nr, Nc, Br, Bc, CM> > {
    typedef ETBase<typename asExpr<TinyBasedMatrix<T, Nr, Nc, Br, Bc, CM> >::T_expr> T_wrapped;
    typedef typename asExpr<TinyBasedMatrix<T, Nr, Nc, Br, Bc, CM> >::T_expr T_expr;
};
...
template<typename T, int N, typename Policy>
struct opType<Array<T,N, Policy> > {
  typedef typename opType<T>::T_optype T_optype;
};
template<typename T, int N>
...
template<typename T, int N, int B>
struct opType<TinyBasedVector<T,N,B> > {
  typedef typename opType<T>::T_optype T_optype;
};
template<typename T, int Nr, int Nc>
...
template<typename T, int Nr, int Nc, int Br, int Bc, bool CM>
struct opType<TinyBasedMatrix<T,Nr,Nc, Br, Bc, CM> > {
  typedef typename opType<T>::T_optype T_optype;
};
