...
Array<P_numtype,N_rank, Policy>::Array(_bz_ArrayExpr<T_expr> expr)
{
...
Array<P_numtype,N_rank, Policy>::Array(const TinyVector<int, N_rank>& lbounds,
 ...
 */
template<typename P_numtype, int N_rank, typename Policy>
_bz_inline2 void Array<P_numtype, N_rank, Policy>::computeStrides()
{
    if (N_rank > 1)
    {
      diffType stride = 1;
...
template<typename P_numtype, int N_rank, typename Policy>
void Array<P_numtype, N_rank, Policy>::calculateZeroOffset()
{
...
template<typename P_numtype, int N_rank, typename Policy>
bool Array<P_numtype, N_rank, Policy>::isStorageContiguous() const
{
...
template<typename P_numtype, int N_rank, typename Policy>
void Array<P_numtype, N_rank, Policy>::dumpStructureInformation(ostream& os) const
{
 ...
 */
template<typename P_numtype, int N_rank, typename Policy>
void Array<P_numtype, N_rank, Policy>::reference(const Array<P_numtype, N_rank, Policy>& array)
{
...
template<typename P_numtype, int N_rank, typename Policy>
void 
Array<P_numtype, N_rank, Policy>::weakReference(const Array<P_numtype, N_rank, Policy>& array)
{
    storage_ = array.storage_;
    length_ = array.length_;
    stride_ = array.stride_;
 ...
 */
template<typename P_numtype, int N_rank, typename Policy>
void Array<P_numtype, N_rank, Policy>::setStorage(GeneralArrayStorage<N_rank> x)
{
#ifdef BZ_DEBUG
    if (size() != 0) {
 ...
 */
template<typename P_numtype, int N_rank, typename Policy>
_bz_inline2 void Array<P_numtype, N_rank, Policy>::setupStorage(int lastRankInitialized)
{
    ...
    if(storage_.padding()==paddedData) {
      ...
      alloc_length[ordering(0)] = 
          static_cast<_bz__int>(simdTypes<T_numtype>::paddedLength(alloc_length[ordering(0)]));
    }
...
template<typename P_numtype, int N_rank, typename Policy>
Array<P_numtype, N_rank, Policy> Array<P_numtype, N_rank, Policy>::copy() const
{
    if (numElements())
    {
        Array<T_numtype, N_rank, Policy> z(length_, storage_);
        z = *this;
        return z;
    }
    else {
...
template<typename P_numtype, int N_rank, typename Policy>
void Array<P_numtype, N_rank, Policy>::makeUnique()
{
...
template<typename P_numtype, int N_rank, typename Policy>
Array<P_numtype, N_rank, Policy> Array<P_numtype, N_rank, Policy>::transpose(int r0, int r1, 
...
template<typename P_numtype, int N_rank, typename Policy>
void Array<P_numtype, N_rank, Policy>::transposeSelf(int r0, int r1, int r2, int r3,
    int r4, int r5, int r6, int r7, int r8, int r9, int r10)
{
    ...
    Array<T_numtype, N_rank, Policy> x(*this);
...
template<typename P_numtype, int N_rank, typename Policy>
void Array<P_numtype, N_rank, Policy>::doTranspose(int destRank, int sourceRank,
    Array<T_numtype, N_rank, Policy>& array)
{
...
template<typename P_numtype, int N_rank, typename Policy>
void Array<P_numtype, N_rank, Policy>::reverseSelf(int rank)
{
...
template<typename P_numtype, int N_rank, typename Policy>
Array<P_numtype, N_rank, Policy> Array<P_numtype,N_rank, Policy>::reverse(int rank)
{
    T_array B(*this);
    B.reverseSelf(rank);
    return B;
...
Array<P_numtype2,N_rank, Policy> Array<P_numtype,N_rank, Policy>::extractComponent(P_numtype2,
 ...
 */
template<typename P_numtype, int N_rank, typename Policy>
_bz_inline2 void Array<P_numtype, N_rank, Policy>::reindexSelf(const 
...
template<typename P_numtype, int N_rank, typename Policy>
_bz_inline2 Array<P_numtype, N_rank, Policy> 
Array<P_numtype, N_rank, Policy>::reindex(const TinyVector<int, N_rank>& newBase) 
{
    T_array B(*this);
