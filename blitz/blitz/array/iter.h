...
private:
    ...
    //
    void Init(const Array<T, N, Policy>& array) {
        ...
        dataincr_(order_(0)) = static_cast<int>(array.stride(order_(0)));
        for(int i = 1, r, s = order_(0); i<N; s = r, ++i) {
            r = order_(i);
            ubound_(i) = array.ubound(i) + 1;
            dataincr_(r) = static_cast<int>(array.stride(r) - array.extent(s)*array.stride(s));
        }
#if defined(BZ_DEBUG)
        beg_ = array.data();
...
public:
    ...
    ConstArrayIterator(const Array<T, N, Policy>& array) :
        data_(const_cast<T*>(array.data())) {
        Init(array);
        pos_ = lbound_;
    }

    ConstArrayIterator(const Array<T, N, Policy>& array, const int) :
    ...
    bool operator==(const ConstArrayIterator<T, N, Policy>& x) const
    {
    ...
    ConstArrayIterator<T, N, Policy>& operator++();
    ConstArrayIterator<T, N, Policy>& operator--();

    ConstArrayIterator<T, N, Policy> operator++(int) {
        ConstArrayIterator<T, N, Policy> tmp = *this;
        ++(*this);
        return tmp;
    }
...
private:
    ...
    static T* end_value(const Array<T, N, Policy>& array) {
...
#if defined(BZ_DEBUG)
    const T* _bz_restrict beg_;
    const T* _bz_restrict end_;
#endif
};


template<typename T, int N, typename Policy>
class ArrayIterator : public ConstArrayIterator<T, N, Policy> {
private:
    typedef ConstArrayIterator<T, N, Policy> T_base;
    using T_base::data_;
...
public:
    ...
    ArrayIterator(Array<T, N, Policy>& x) : T_base(x) { }

    ArrayIterator(const Array<T, N, Policy>& array, const int) : T_base(array, 0) { }
    ...
    T* _bz_restrict operator->() const {
        CheckIteratorValidity("Attempted to dereference", 0);
        return data_;
    }

    ArrayIterator<T, N, Policy>& operator++() {
        T_base::operator++();
        return *this;
    }

    ArrayIterator<T, N, Policy> operator++(int) {
        ArrayIterator<T, N, Policy> tmp = *this;
        ++(*this);
        return tmp;
    }
...
template<typename T, int N, typename Policy>
ConstArrayIterator<T, N, Policy>& ConstArrayIterator<T, N, Policy>::operator++() {
...
template <typename T, int N, typename Policy>
struct iterator_traits< BZ_BLITZ_SCOPE(ConstArrayIterator)<T, N, Policy> > {
...
template <typename T, int N, typename Policy>
struct iterator_traits< BZ_BLITZ_SCOPE(ArrayIterator)<T, N, Policy> > {
    typedef bidirectional_iterator_tag         iterator_category;
    typedef T                                  value_type;
    typedef blitz::diffType                    difference_type;
    typedef T*                                 pointer;
