...
public:
    ...
    constexpr StaticArrayStorage(noInitializeFlag)
    { }

    constexpr StaticArrayStorage(paddingPolicy pp = defaultPadding)
    {
 //       for (int i=0; i < N_rank; ++i)
 //         ordering_(i) = N_rank - 1 - i;
 //       ascendingFlag_ = true;
 //       base_ = 0;
    ...
    constexpr StaticArrayStorage(const StaticArrayStorage<N_rank>& x)
 //     : paddingPolicy_(x.paddingPolicy_), ascendingFlag_(x.ascendingFlag_),
	//ordering_(x.ordering_), base_(x.base_)
    { 
    }

    constexpr StaticArrayStorage(TinyVector<int,N_rank> ordering,
			TinyVector<bool,N_rank> ascendingFlag,
			paddingPolicy pp = defaultPadding)
 //     : paddingPolicy_(pp), ascendingFlag_(ascendingFlag),
	//ordering_(ordering)
    ...
    constexpr StaticArrayStorage<N_rank>& operator=(
        const StaticArrayStorage<N_rank>& rhs) const
    {
 //       ordering_ = rhs.ordering();
 //       ascendingFlag_ = rhs.ascendingFlag();
 //       base_ = rhs.base();
    ...
    constexpr TinyVector<int, N_rank> ordering() const
    {
        return TinyVector<int, N_rank>{0};
    }

    constexpr int ordering(int i) const
    { return 0; }

    //void setOrdering(int i, int order) 
    //{ ordering_[i] = order; }

    constexpr bool allRanksStoredAscending() const
    {
        //for (int i=0; i < N_rank; ++i)
        //    if (!ascendingFlag_[i]) return false;
        return true;
    ...
    constexpr bool isRankStoredAscending(int i) const
    { return true; }

    //constexpr TinyVector<bool, N_rank>& ascendingFlag()
    //{ return ascendingFlag_; }

    constexpr TinyVector<bool, N_rank> ascendingFlag() const
    { return         return TinyVector<bool, N_rank>{true};
    ; }
    ...
    constexpr TinyVector<int, N_rank> base() const
    { return TinyVector<int, N_rank>{1}; }

    constexpr int base(int i) const
    { return 1; }
    ...
    constexpr paddingPolicy padding() const { return contiguousData; }
