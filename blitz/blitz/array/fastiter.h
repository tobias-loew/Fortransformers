...
// forward declaration
template<typename, int, typename> class FastArrayIterator;
template<typename, int, typename> class FastArrayCopyIterator;


template<typename P_numtype, int N_rank, typename Policy, typename P_arraytype>
...
public:
  ...
  typedef FastArrayIteratorBase<T_numtype, N_rank, Policy, P_arraytype> T_iterator;
    typedef const T_array& T_ctorArg1;
    typedef int            T_ctorArg2;    // dummy
  typedef FastArrayCopyIterator<T_numtype, N_rank, Policy> T_range_result;
  ...
  { return array_; }
    ...
    int stride() const
    {
        return static_cast<_bz__int>(stride_);
    }

  /** Returns true if the Array has unit stride in the rank. */
    constexpr bool isUnitStride(int rank) const
  ...
  class SliceInfo {
  public:    
    typedef FastArrayCopyIterator<P_numtype, blitz::SliceInfo<P_numtype, Policy, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11>::rank, Policy> T_slice;
  };
...
template<typename P_numtype, int N_rank, typename Policy> class FastArrayCopyIterator;

template<typename P_numtype, int N_rank, typename Policy>
class FastArrayIterator : 
  public FastArrayIteratorBase<P_numtype, N_rank, Policy, const Array<P_numtype, N_rank, Policy>&>
{
public:
  typedef FastArrayIteratorBase<P_numtype, N_rank, Policy,
				const Array<P_numtype, N_rank, Policy>&> T_base;
  ...
  // NB: this ctor does NOT preserve stack and stride
  // parameters.  This is for speed purposes.
  FastArrayIterator(const FastArrayIterator<P_numtype, N_rank, Policy>& x)
    : T_base(x)
  { }
  ...
  void operator=(const FastArrayIterator<P_numtype, N_rank, Policy>& x)
  {
    T_base::operator=(x);
  }
...
public:
  ...
  // NB: this ctor does NOT preserve stack and stride
  // parameters.  This is for speed purposes.
  FastArrayCopyIterator(const FastArrayCopyIterator<P_numtype, N_rank, Policy>& x)
    : T_base(x)
  { }
