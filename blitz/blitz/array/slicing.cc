...
void Array<P_numtype, N_rank, Policy>::constructSubarray(PolicyT,
    Array<T_numtype, N_rank, Policy>& array, const RectDomain<N_rank>& subdomain)
{
    reference(array);
...
template<typename PolicyT>
void Array<P_numtype, N_rank, Policy>::constructSubarray(PolicyT,
    Array<T_numtype, N_rank, Policy>& array, const StridedDomain<N_rank>& subdomain)
{
    reference(array);
    for (int i=0; i < N_rank; ++i)
        slice(i, subdomain[i]);
...
template<typename PolicyT>
void Array<P_numtype, N_rank, Policy>::constructSubarray(PolicyT,
    Array<T_numtype, N_rank, Policy>& array, Range r0)
{
    reference(array);
    slice(0, r0);
}
...
template<typename PolicyT>
void Array<P_numtype, N_rank, Policy>::constructSubarray(PolicyT,
    Array<T_numtype, N_rank, Policy>& array, Range r0, Range r1)
{
    reference(array);
    slice(0, r0);
    slice(1, r1);
...
template<typename PolicyT>
void Array<P_numtype, N_rank, Policy>::constructSubarray(PolicyT,
    Array<T_numtype, N_rank, Policy>& array, Range r0, Range r1, Range r2)
{
    reference(array);
    slice(0, r0);
    slice(1, r1);
...
template<typename PolicyT>
void Array<P_numtype, N_rank, Policy>::constructSubarray(PolicyT,
    Array<T_numtype, N_rank, Policy>& array, Range r0, Range r1, Range r2, Range r3)
{
    reference(array);
    slice(0, r0);
    slice(1, r1);
 ...
 */
template<typename P_numtype, int N_rank, typename Policy> template<int N_rank2, typename R0,
    class R1, typename R2, typename R3, typename R4, typename R5, typename R6, typename R7,
    class R8, typename R9, typename R10>
void Array<P_numtype, N_rank, Policy>::constructSlice(Array<T_numtype, N_rank2, Policy>& array,
 ...
 * Range parameters.
 */
template<typename P_numtype, int N_rank, typename Policy> template<int N_rank2>
void Array<P_numtype, N_rank, Policy>::slice(int& setRank, Range r,
    Array<T_numtype,N_rank2, Policy>& array, TinyVector<int,N_rank2>& rankMap,
    int sourceRank)
{
 ...
 */
template<typename P_numtype, int N_rank, typename Policy> template<int N_rank2>
void Array<P_numtype, N_rank, Policy>::slice(int&, int i,
    Array<T_numtype,N_rank2, Policy>& array, TinyVector<int,N_rank2>& rankMap,
    int sourceRank)
{
#ifdef BZ_DEBUG_SLICE
 ...
 */
template<typename P_numtype, int N_rank, typename Policy>
void Array<P_numtype, N_rank, Policy>::slice(int rank, Range r)
{
    ...
    BZPRECHECK((((first <= last+1) && (stride > 0)) || 
        ((first >= last-1) && (stride < 0))) && 
        (first >= base(rank) && (first - base(rank)) <= length_[rank] || length_[rank] == -1) &&
        (last >= base(rank)-1 && (last - base(rank)) < length_[rank] || length_[rank] == -1),
    ...
    length_[rank] = static_cast<_bz__int>((last - first) / stride + 1);
...
template<typename P_numtype, int N_rank, typename Policy>
template<int N_base>
void Array<P_numtype, N_rank, Policy>::constructSubarray(array_policy::StaticFortranArrayPolicy<N_base>,
    Array<T_numtype, N_rank, Policy>& array, Range r0) {

    data_ = array.data_ + r0.first(N_base) - N_base;
    length_[0] = r0.length();
