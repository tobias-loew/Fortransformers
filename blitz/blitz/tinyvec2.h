...
public:
  ...
  typedef FastTV2CopyIterator<P_numtype, N_length> T_range_result;
    ...
    TinyVector() = default; // defaulting enables zero-initialization of aggregates
    //TinyVector()  { }
    //~TinyVector() { }
  ...
  constexpr static bool lengthCheck(unsigned i) 
    {
      BZ_CONSTEXPR_PRECHECK(i < N_length,
            "TinyVector<" << BZ_DEBUG_TEMPLATE_AS_STRING_LITERAL(T_numtype) 
            << "," << N_length << "> index out of bounds: " << i);
        return true;
    }

  constexpr const T_numtype& operator()(unsigned i) const
    {
