...
#ifndef BZ_TBMEVALUATE_H
#define BZ_TBMEVALUATE_H
...
/** The _tbm_evaluator class has a bool template argument that is used
    to select code paths at compile time.  */
template<bool unroll, int N_rows, int N_columns, int N_rows_base, int N_columns_base, bool B_column_major>
struct _tbm_evaluator {

  /** The select_evaluation function redirects expressions that do not
      contains solely TinyBasedMatrix operands to the general evaluation
      ...
      TinyBasedMatrix-only evaluation. Since TinyBasedMatrixs can't have funny
  ...
  select_evaluation(TinyBasedMatrix<T, N_rows, N_columns, N_rows_base, N_columns_base, B_column_major>& dest,
  ...
  _tbm_evaluator<do_unroll, N_rows, N_columns, N_rows_base, N_columns_base, B_column_major>::evaluate_aligned(dest.data(), expr, T_update());
  }

  /** This version of the evaluation function assumes that the
      TinyBasedMatrixs have appropriate alignment (as will always be the
      case if they are actual TinyBasedMatrix objects and not created using
...
/** Specialization of the _tbm_evaluator class for false template arguments. */
template<int N_rows, int N_columns, int N_rows_base, int N_columns_base, bool B_column_major>
struct _tbm_evaluator<true, N_rows, N_columns, N_rows_base, N_columns_base, B_column_major> {

  /** The false version of select_evaluation is picked for expressions
      that contain operands other than TinyBasedMatrixs. It just redirects
...
/** This function selects evaluation path by calling select_evaluation
    ...
    contains TinyBasedMatrix operands. */
...
_bz_forceinline
void
TinyBasedMatrix<P_numtype,N_rows, N_columns, N_rows_base, N_columns_base, B_column_major>::_tbm_evaluate(const T_expr& expr, T_update)
{
  const bool mixed_expr =
    (T_expr::numArrayOperands>0) || 
    (T_expr::numTVOperands>0) ||
    (T_expr::numIndexPlaceholders>0);
  _tbm_evaluator<mixed_expr, N_rows, N_columns, N_rows_base, N_columns_base, B_column_major>::select_evaluation(*this, expr, T_update());
}


BZ_NAMESPACE_END

#endif // BZ_TBMEVALUATE_H
