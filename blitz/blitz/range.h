  ...
  int length(int =0) const
    {
        ...
        return static_cast<_bz__int>((last_ - first_) / stride_ + 1);
    }
...
// class indicating explicitely that it's using strides
// for performance reasons used in tinybasedvec2.h
class Stride {
    Range rng;

public:
    Stride(int first, int last, diffType stride)
        : rng(first, last, stride)
    {}

    Range const& get_range() const { return rng; }
};
