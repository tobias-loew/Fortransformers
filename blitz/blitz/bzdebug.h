...
_bz_global int  assertSuccessCount BZ_GLOBAL_INIT(0);
  ...
  inline void endCheckAssert()
  {
    ...
    #define BZ_CONSTEXPR_PRECHECK(X,Y) BZPRECHECK(X,Y)
...
// from http://ericniebler.com/2014/09/27/assert-and-constexpr-in-cxx11/
...
*/
    ...
    #define BZPRECHECK(X,Y)                                                 \
        { if (!(X))                                                         \
          { std::stringstream s; s << "[Blitz++] Precondition failure: Module " \
                               << __FILE__                                  \
...
#define BZ_CONSTEXPR_PRECHECK(X,Y)                                                 \
        { if (!(X))                                                         \
          { throw assert_failure( [&]{std::stringstream s; s << "[Blitz++] Precondition failure: Module " \
                               << __FILE__                                  \
               << " line " << __LINE__ << BZ_STD_SCOPE(endl)                \
               << Y << BZ_STD_SCOPE(endl);                                  \
            OutputDebugStringA(s.str().c_str());                                     \
    ...
    #define BZ_DEBUG_MESSAGE(X)                                    \
        { std::stringstream s; s  << __FILE__ << ":" << __LINE__ << " " \
                             << X << BZ_STD_SCOPE(endl); OutputDebugStringA(s.str().c_str()); }
#else
    #define BZPRECHECK(X,Y)                                                 \
            { if (!(X))                                                         \
...
#define BZ_CONSTEXPR_PRECHECK(X,Y)                                                 \
        { if (!(X))                                                         \
          { throw assert_failure( [&]{BZ_STD_SCOPE(cerr)  << "[Blitz++] Precondition failure: Module " \
                               << __FILE__                                  \
               << " line " << __LINE__ << BZ_STD_SCOPE(endl)                \
               << Y << BZ_STD_SCOPE(endl);                                  \
                BZ_STD_SCOPE(cerr).flush();                                     \
