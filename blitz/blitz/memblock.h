...
private:   // Data members 
 ...
 union {
   ...
   typename simdTypes<T_type>::vecType * _bz_restrict data_tv_;
   char * _bz_restrict        data_char_;
  };
 union {
   T_type *              dataBlockAddress_;
   typename simdTypes<T_type>::vecType * _bz_restrict dBA_tv_;
   char *                dBA_char_;
 };
  sizeType              length_;
...
template<typename P_type>
class StaticMemoryBlockReference {
...
public:
    ...
    StaticMemoryBlockReference() {
        // no block, so nothing to add reference to
        data_ = 0;
    }

    StaticMemoryBlockReference(StaticMemoryBlockReference<T_type>& ref, sizeType offset = 0) {
        data_ = ref.data_ + offset;
    }

    StaticMemoryBlockReference(T_type* data) {
        data_ = data;
    }

    StaticMemoryBlockReference(sizeType , T_type* data,
...
//            }
            ...
            void changeBlock(StaticMemoryBlockReference<T_type>& ref, sizeType offset = 0) {
                //blockRemoveReference();
                //block_ = ref.block_;
                //addReference();
                data_ = ref.data_ + offset;
...
//            };
            ...
            void operator=(const StaticMemoryBlockReference<T_type>&) {}
        };
