...
char *h_functions = "h_functions";
std::string proto_fname	= "proto_file";
...
Un_link_all(int cdelete)
{
	if (!debugflag) {
		...
		_unlink(sortfname);
		_unlink(blkdfname);
		if (cdelete && !coutput.empty())
			_unlink(coutput.c_str());
		}
	}
...
set_tmp_names()
{
	...
	tmpstring=(char*)malloc(_MAX_PATH+1);

	if(!tmpstring)
		strcpy(tmpdir,".\\");
	else
	{
		k=GetTempPath(_MAX_PATH+1,tmpstring);
		if(!k) strcpy(tmpdir,".\\");
		else 
		{
			tmpstring[k-1]='\0';
			tmpdir=(char*)malloc(k);
			if(!tmpdir) strcpy(tmpdir,".\\");
			else
			{
				strcpy(tmpdir,tmpstring);free(tmpstring);
			}
		}
	}

// WIN95 doesn't like the '/'
	...
	if (debugflag == 1)
		return;
	k = strlen(tmpdir) + _MAX_PATH;
	c_functions = (char *)ckalloc(8*k);
    ...
    sprintf(h_functions, "%s\\f2c%d_h", tmpdir, pid);
...
std::string c_name(std::string s, std::string ft) {
    ...
    {
        Fatal("file name must have . ");
    }

    return s.substr(0, pos + 1) + ft;
 }
...
#ifdef WIN32
	Fatal("Only one Fortran input file allowed under WIN32");
#else
