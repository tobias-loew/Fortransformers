...
bool global_force_upper_case = false;

static void
write_indent(FILE *fp, int use_indent, int extra_indent, char *start, char *end, bool add_comment)
 //FILE *fp;
...
/*VARARGS2*/
void margin_printf(FILE *fp, const char *a, long b, long c, long d, long e, long f, long g)
//FILE *fp;
//char *a;
//long b, c, d, e, f, g;
{
...
/*VARARGS2*/
void nice_printf(FILE *fp, const char *a, long b, long c, long d, long e, long f, long g)
//FILE *fp;
//char *a;
//long b, c, d, e, f, g;
{
...
#define  max_comment_line_len c_comment_output_line_length
...
static char *
adjust_pointer_in_string(char *pointer)
...
int ind_printf(int use_indent, FILE *fp, const char *a, long b, long c, long d, long e, long f, long g)
//int use_indent;
//FILE *fp;
//char *a;
//long b, c, d, e, f, g;
{
    ...
    extern int max_comment_line_len;
    extern FILEP c_file;
    ...
    if(global_force_upper_case)
    {
        int len = strlen(next_slot);
        for(int i = 0; i < len; i++)
        {
            next_slot[i] = toupper(next_slot[i]);
        }
    }
    ...
    /* The   for   loop will parse one output line */
        ...
        if(in_comment)
        {
            boost::optional<int> last_blank_cursor_pos;
            char *last_blank_pointer;
            for(pointer = next_slot; *pointer && *pointer != '\n' &&
                cursor_pos <= max_comment_line_len; pointer++)
            {
                if(*pointer == ' ' || *pointer == '\t')
                {
                    last_blank_pointer = pointer;
                    last_blank_cursor_pos = cursor_pos;
                }
                cursor_pos++;
            }
            if(cursor_pos > max_comment_line_len && last_blank_cursor_pos)
            {
                pointer = last_blank_pointer;
                cursor_pos = *last_blank_cursor_pos;
            }
            ++in_comment;
        }
        else
            for(pointer = next_slot; *pointer && *pointer != '\n' &&
                ...
                case '\'':
                    ...
                    break;
                case '\t':
                    cursor_pos = tab_length * ((cursor_pos + tab_length) / tab_length) - 1;
                    break;

                case ':':
                    if(pointer[1] == ':' && cursor_pos + 2 > max_line_len) {
                        //word_start = 0;
                        --pointer;
                        goto overflow;
                    }
                ...
                case '*':
                    ...
                    // drop thru, as we don't know better
    ...
    */
            ...
            last_char = *pointer;
            write_indent(fp, use_indent, extra_indent, output_buf, pointer, in_comment > 2);
...
int ind_printf(int use_indent, FILE *fp, const char *a, ...)
{
    ...
    int result = ind_printf(use_indent, fp, a,
        arg[0],
        arg[1],
        arg[2],
        arg[3],
