	...
	for(; p ; p = p->nextp)
	{
        ...
        if(cxx) {
            if(dp = q->vdim)	/* if this param is an array ... */
            {
                ...
                for(i = 0; i <= nd; ++i)
                {
                    if(!is_const(dp->dims[i].lb) || !is_const(dp->dims[i].ub))
                    {
                        adjust = true;
                        break;
                    }
                ...
                if(adjust)
                {
                    ...
                    nice_printf(outfile, "auto __arrray_adjust__%s = ft::make_arrray_adjuster(%s",
                        q->cvarname, q->cvarname
                    );
                        
                    for(i = 0; i <= nd; ++i)
                    {
                        nice_printf(outfile, ", ft::dyn_bounds(");
                        if(dp->dims[i].lb) {
                            expr_out(outfile, cpexpr(dp->dims[i].lb));
                        } else {
                            nice_printf(outfile, "1");
                        }
                        if(dp->dims[i].ub)
                        {
                            nice_printf(outfile, ", ");
                            expr_out(outfile, cpexpr(dp->dims[i].ub));
                        }
                        else {
                            nice_printf(outfile, ", ft::dyn_unbounded");
                            if (i != nd) {
                                nice_printf(outfile, " PLEASE CHECK /* unbounded should only be used with the rightmost dimension*/");
                            }
                        }
                        nice_printf(outfile, ")");
                    }
