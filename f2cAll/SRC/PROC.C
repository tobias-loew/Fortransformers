...
LOCAL void dobss(), docomleng(), docommon(),
...
LOCAL void retval(proctype_t t);
void donmlist();
LOCAL void dobss();
void namelist(Namep np);

bool redirect_to_proto = false;

static char Blank[] = BLANKCOMMON;

 static char *postfix[] = { "g", "h", "i", "r", "d", "c", "z", "g", "h", "i" };

 std::string var_derived_type_name; // for variable declaration
 std::string var_derived_type_extends_name; // for variable declaration
 std::string type_derived_type_name; // for type definition
 bool in_type_definition = false;
std::vector<derived_type> new_types;

 chainp new_procs;
 int prev_proc, proc_argchanges, proc_protochanges;

extern struct Dims dims[MAXDIM + 1];
extern int ndim;
...
fix_entry_returns()	/* for multiple entry points */
{
 ...
 proctype_t make_proctype(Namep np)
 {
     return std::make_tuple(np, np->derived_type_name ? np->derived_type_name : std::string(), np->vtype);
 }
 static void
putentries(FILE *outfile)	/* put out wrappers for multiple entries */
 //FILE *outfile;
{
	char base[IDENT_LEN];
	struct Entrypoint *e;
	...
	do {
        ...
        type = np->vtype;
        proctype = make_proctype(np);
		if (protofile)
			protowrite(protofile, proctype, np->cvarname, e, lengths);
		nice_printf(outfile, "\n%s ", cpp_type_decl(np, 1, false).c_str());
...
void newpart(int what)
{
    // ignore interface definitions
...
void newmodule()
{
    newpart(CLMODULEHEAD);
    redirect_to_proto = true;
//    p1put(P1_REDIRECT_TO_PROT_START);
}
...
/* Write out the abbreviations for common block reference */
    ...
    if(procclass == CLUNKNOWN && !module_name.empty())
    {
//        exendmodule();
    ...
    else
    {
        ...
        donmlist();
        dobss();
        start_formatting (procclass == CLMODULEHEAD);
        if (nentry > 1)
            putentries(c_file);
        zap_changes();
...
//int class_;
{
	...
	procclass = class_;
	fprintf(diagfile, "   %s", (class_==CLMAIN ? "MAIN" : "BLOCK DATA") );
	if(progname) {
 ...
 //int substmsg;
{
    ...
    p = mkext(v->fvarname, addunder(v->cvarname, false));
...
void entrypt(int class_, int type_, ftnint length, Extsym *entry, chainp args, chainp postfixes)
//int class_, type;
//ftnint length;
//Extsym *entry;
//chainp args;
{
    ...
    int type = GET_TYPE(type_);
    bool is_pure = !!(type_ & TY_PREFIX_PURE);
    bool is_elemental = !!(type_ & TY_PREFIX_ELEMENTAL);
	...
	if(class_ != CLENTRY)
		puthead( procname = entry->cextname, class_);
	else
	...
	{
		procclass = CLPROC;
		proctype = std::make_tuple(nullptr, var_derived_type_name, type);
    ...
    if(postfixes && postfixes->datap) {
        expptr name = (expptr)postfixes->datap;
        if(name->tag == TNAME) {
            p->alt_result = name->nameblock.fvarname;
        }
    }
	...
	{
		class_ = CLPROC;
		if(std::get<2>(proctype) == TYSUBR)
			type = TYSUBR;
	}

	q->vclass = class_;
	q->vprocclass = 0;
	settype(q, type, mkintcon(length), 0);
...
/* Return a zero only when the alternate return mechanism has been
   specified in the function header */
			...
			if (!cxx && (substars || Ansi) && lastwasbranch != YES)
			    p1_subr_ret (ICON(0));
		}
...
//register int t;
{
    ...
    case TYINTPTR:
		...
		p->vtype = std::get<2>(t);
		p1_subr_ret (mkconv (std::get<2>(t), fixtype((expptr)p)));
		break;
    ...
    case TY_DERIVED:
        {
            p = (Addrp)cpexpr((expptr)retslot);
            p->vtype = std::get<2>(t);
            p->derived_type_name = _strdup(std::get<1>(t).c_str());
            p1_subr_ret(fixtype((expptr)p));
        }
        break;

	default:
		badtype("retval", std::get<2>(t));
	}
}
...
/* So now we're working with something other than CLMAIN or CLBLOCK.
   Determine the type of its return value. */
    ...
    if(!ep->alt_result.empty()) {
        char* s = _strdup(ep->alt_result.c_str());
        
        Namep alt_np = mkname(s);
        type = alt_np->vtype;
        std::get<0>(proctype) = alt_np;
        std::get<2>(proctype) = type;
    }
    ...
    if(cxx)
    {
        if(!std::get<0>(proctype))
        {
            std::get<0>(proctype) = np;
            std::get<2>(proctype) = type;
        }
    }
    else
    {
        if(std::get<2>(proctype) == TYUNKNOWN)
            if( (std::get<2>(proctype) = type) == TYCHAR)
	...
	else if(type != std::get<2>(proctype))
		multitype = YES;
	if(rtvlabel[type] == 0)
...
/* Put a new argument in the function, one which will hold the result of
   ...
   mkarg(). */
		...
		if (!xretslot[TYCHAR]) {
			xretslot[TYCHAR] = rs =
				autovar(0, proctype, ISCONST(np->vleng)
					? np->vleng : ICON(0), "", false);
...
/* Handle a   complex   return type -- declare a new parameter (pointer to
   a complex value) */
		...
		if (!xretslot[type])
			xretslot[type] =
				autovar(0, proctype, EXNULL, " ret_val", true);
		...
		if (type == TYUNKNOWN) {
            ...
            else
            {
                ...
                std::get<2>(proctype) = type = np->vtype =
                    dflttype[letter(np->fvarname[0])];
            }
		}
		if (!xretslot[type])
			xretslot[type] = retslot =
				autovar(1, proctype, EXNULL, " ret_val", true);
				/* the blank is for use in out_addr */
		np->vstg = STGAUTO;
		}
...
/* Give external procedures the proper storage class_ */

			else if(qclass==CLPROC && q->vprocclass==PEXTERNAL
					&& qstg!=STGARG) {
				e = mkext(q->fvarname,addunder(q->cvarname, false));
				e->extstg = STGEXT;
...
//chainp list;
{
	...
	if (namep -> vdim) {
	    nd = namep -> vdim -> ndim;
	    size = sizeof(int) + (3 + Dimblock::dim_expptr_count * nd) * sizeof (expptr);
	    ...
	    for (i = 0; i < nd; i++) {
            ...
            dp->dims[i].lb = (expptr)cpexpr(dp->dims[i].lb);
            dp->dims[i].ub = (expptr)cpexpr(dp->dims[i].ub);
        } /* for */
	} /* if */
...
//char *name;
{
	...
	if(!cxx && std::get<2>(t) == TYCHAR)
		...
		leng = typesize[std::get<2>(t)];

	q = ALLOC(Addrblock);
    q->is_retval = (is_retval ? 1 : 0);
	q->tag = TADDR;
	q->vtype = std::get<2>(t);
	if(!cxx && std::get<2>(t) == TYCHAR)
	{
    ...
    if(std::get<2>(t) == TY_DERIVED)
    {
        q->derived_type_name = _strdup(std::get<1>(t).c_str());
    }
    ...
    //            q->vdim = p = (struct Dimblock *)
    //                ckalloc(sizeof(int) + (3 + Dimblock::dim_expptr_count*n->ndim) * sizeof(expptr));
    //            p->ndim = nd--;
    //            p->nelt = ICON(1);
    //            doin_setbound = 1;
    ...
    //            for(i = 0; i <= nd; ++i)
    //            {
    //                p->dims[i].lb = cpexpr(dims[i].lb);
    //                p->dims[i].ub = cpexpr(dims[i].ub);
    //                if((q = dims[i].ub) == NULL)
    //                {
    //                    if(i == nd)
    //                    {
    ...
    //                                autovar(1, std::make_tuple(nullptr, "", tyint), EXNULL, buf);
    //                        }
    //                        else
    //                        {
    //                            p->dims[i].dimsize = cpexpr(q);
    //                        }
    //                        p->dims[i].dimexpr = q;
    //                        if(i == nd)
    //                        {
    ...
    //                    p->baseoffset = (expptr)autovar(1, std::make_tuple(nullptr, "", tyint), EXNULL, buf);
    //                }
    //                else
    //                {
    //                    p->baseoffset = cpexpr(q);
    //                }
    //                p->basexpr = q;
    //                v->vdimfinish = 1;
    //            }
    ...
    if (*name == ' ')
        ...
        q->uname_tag = UNAM_IDENT;
        temp_name(av_pfix[std::get<2>(t)], ++autonum[std::get<2>(t)], q->user.ident);
    }
  //  if (nelt0 > 0)
        declare_new_addr (q);
 ...
 */
	...
	q = autovar(nelt, std::make_tuple(nullptr, "", type), lengp, "", false);
	return(q);
}
...
/* settype -- set the type or storage class_ of a Namep object.  If
...
{
    settype(v, type.first, length_ptr, type.second);
}
...
//register ftnint length;
{
    ...
    int length = 0;
    if(length_ptr) {
        if(ISICON(length_ptr)) {
            length = length_ptr->constblock.Const.ci;
        } else {
            // check for *N(M[,K]...) style in character fields
            if(length_ptr->tag == TEXPR && length_ptr->exprblock.opcode == OPCALL) {
                switch(length_ptr->exprblock.opcode) {
                case OPCALL:
                    {
                        switch(length_ptr->exprblock.rightp->tag) {
                        case TLIST:
                            {
                                ndim = 0;
                                auto lb = length_ptr->exprblock.rightp->listblock;
                                for(auto q = lb.listp; q; q = q->nextp) {
                                    if(auto e = (expptr)q->datap) {
                                        switch(e->tag) {
                                        case TSPECIALLIST:
                                            {
                                                assert(e->listblock.vtype == VTSPECIALLIST_SUBARRAY);
                                                auto listp = &e->listblock;
                                                expptr list[3] = {
                                                    (expptr)listp->listp->datap,
                                                    (expptr)listp->listp->nextp->datap,
                                                    (expptr)listp->listp->nextp->nextp->datap,
                                                };
                                                dims[ndim].lb = list[0];
                                                dims[ndim].ub = list[1];
                                                assert(list[2] == 0);
    ...
    if(type == TYDREAL && length == 4)
    {
        type = TYREAL;
    }


    if((attribute&ATTRIBUTE_ARG_MASK) != 0)
    {
        if(v->vstg != STGARG){
//            dclerr("intent/value on non-argument", v);
        }
        if(attribute & (ATTRIBUTE_IN | ATTRIBUTE_INOUT)){
            v->intent_in = 1;
        }
        if(attribute & (ATTRIBUTE_OUT | ATTRIBUTE_INOUT)){
            v->intent_out = 1;
        }
        if(attribute & ATTRIBUTE_VALUE){
            v->intent_value = 1;
        }
        if(v->intent_out && v->intent_value){
            dclerr("value cannot go with out", v);
        }
    }

    if(attribute&ATTRIBUTE_OPTIONAL)
    {
        v->is_optional = 1;
    }

    if(attribute&ATTRIBUTE_EXTERNAL)
    {
        v->is_external = 1;
    }

    if(attribute&ATTRIBUTE_ALLOCATABLE)
    {
        v->is_allocatable = 1;
    }
	...
	else if(type < 0)	/* storage class_ set */
	{
	...
	else if(v->vtype == TYUNKNOWN || v->vimpltype && v->vtype != type)
	{
        ...
        if(v->vtype == TY_DERIVED)
        {
            //v->derived_type = new derived_type{};
            //v->derived_type->name = var_derived_type_name;

            v->derived_type_name = _strdup(var_derived_type_name.c_str());
        }
			...
			if (v->vstg == STGEXT
			 ...
			 &&  type1 != v->vtype)
				...
				xretslot[type] = autovar(ONEOF(type,
					MSKCOMPLEX|MSKCHAR) ? 0 : 1, std::make_tuple(nullptr, var_derived_type_name, type),
					v->vleng, " ret_val", true);
				if (procclass == CLMAIN)
					errstr(
...
//struct Dims dims[ ];
{
	...
	v->vdim = p = (struct Dimblock *)
	    ckalloc( sizeof(int) + (3+ Dimblock::dim_expptr_count*nd)*sizeof(expptr) );
	p->ndim = nd--;
	p->nelt = ICON(1);
	doin_setbound = 1;
		...
		else
		{
			...
			if( ISCONST(q) )
			{
                ...
                if(!cxx)
                {
                    sprintf(buf, " %s_dim%d", v->fvarname, i+1);
                    p->dims[i].dimsize = (expptr)
                        autovar(1, std::make_tuple(nullptr, "", tyint), EXNULL, buf, false);
                }
                else
                {
                    p->dims[i].dimsize = cpexpr(q);
	...
	else
	{
        ...
        {
            sprintf(buf, " %s_offset", v->fvarname);
            p->baseoffset = (expptr) autovar(1, std::make_tuple(nullptr, "", tyint), EXNULL, buf, false);
        }
        else
        {
            p->baseoffset = cpexpr(q);
...
void endtype()
{
    ...
    for(auto entry = type_hashtab; entry < type_lasthash; ++entry) {
        auto var = entry->varp;

        if(var && var->is_toplevel) {
            names.push_back(var);
        }
    }
    ...
    new_types.emplace_back();

    auto&& t = new_types.back();
    t.name = type_derived_type_name;
    t.name_super = var_derived_type_extends_name;
    for(auto&& n : names)
    {
        t.members.push_back(n);
    }


//    p1putds(P1_STRUCT, names.size(), type_derived_type_name.c_str());
//
//
//    for(auto&& n : names)
//    {
////        p1_expr((expptr)n);
//        p1_name(n);
////        p1putn(P1_ADDR, (int)sizeof(struct Addrblock), (char *)n);
//    }

    in_type_definition = false;
    type_derived_type_name.empty();
    var_derived_type_extends_name.empty();

    type_hashclear();

    if(parstate == INTYPEDCL)
    {
        for(auto&& t : new_types)
        {
            p1putdss(P1_STRUCT, t.members.size(), t.name.c_str(), t.name_super.c_str());
