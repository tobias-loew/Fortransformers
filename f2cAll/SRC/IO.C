...
LOCAL void ioset(int type, int offset, expptr p);
LOCAL void dofmove(char *subname);
LOCAL void putio(expptr nelt, expptr addr);
LOCAL void dofclose(), dofinquire(), dofinquire(), dofopen(),
doiolist();
LOCAL void iosetip(int i, int offset);
LOCAL void iosetlc(int i, int offp, int offl);
LOCAL void putiocall(expptr q);
LOCAL void ioseta(int offset, Addrp p);
LOCAL void iosetc(int offset, expptr p);
	...
	if(iostmt!=IOREAD && iostmt!=IOWRITE)
	{
		...
		if(ioblkp == NULL)
			io_structs[iostmt] = ioblkp =
				autovar(1, std::make_tuple(nullptr, "", ios->type), ENULL, "", false);
 ...
 //register expptr addr;
{
	...
	if(type != TYCHAR)
	{
		...
		c->charleng = 1;
		sprintf(c->user.ident, "(ftnlen)sizeof(%s)", typename_[type]);
 ...
 //char *subname;
{
	...
	if( (p = V(IOSUNIT)) && (cxx || ISINT(p->headblock.vtype)) )
	{
		ioset(TYIOINT, XUNIT, cpexpr(p) );
...
static int ioset_assign = OPASSIGN;
 ...
 LOCAL void
ioset(int type, int offset, expptr p)
 //int type, offset;
 //register expptr p;
{
	offset /= SZLONG;
