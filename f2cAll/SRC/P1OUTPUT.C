...
static void p1_list(struct Listblock *listp);
static void p1_binary(struct Exprblock *e);
static void p1_big_addr(Addrp addrp);
static void p1_literal(long memno);
static void p1_unary(struct Exprblock *e);
static void p1_addr(struct Addrblock *addrp);
static void p1_const(Constp cp);
static void p1putd ( int, long );
 ...
 //register struct Addrblock *addrp;
{
    ...
    if (ONEOF(stg, M(STGUNKNOWN) | M(STGINIT) | M(STGREG))
	...
	|| ONEOF(stg, M(STGBSS)|M(STGINIT)|M(STGAUTO)|M(STGARG)) &&
		(!addrp->memoffset || !ISICON(addrp->memoffset)
			|| addrp->memoffset->constblock.Const.ci)
...
void p1_head(int class_, char *name)
//int class;
//char *name;
{
    p1putds(P1_HEAD, class_, name ? name : "");
} /* p1_head */
...
void p1_for(expptr init, expptr test, expptr inc, expptr limit)
{
    p1put(P1_FOR);
    p1_expr(init);
    p1_expr(test);
...
void p1for_end(bool needs_breakout_for, int loops_count)
{
    assert(loops_count > 0);
    p1putdd(P1_ENDFOR, needs_breakout_for, loops_count);
} /* p1for_end */


void p1_switch(expptr value)
{
    p1put(P1_SWITCH);
    p1_expr(value);
}
...
void p1_namespace(const char* name)
{
    p1puts(P1_NAMESPACE, name);
}


void p1namespace_end(const char* name)
{
    p1puts(P1_ENDNAMESPACE, name);
}

void p1_include(const char* name)
{
    p1puts(P1_INCLUDE, name);
}

void p1_using_namespace(const char* name)
{
    p1puts(P1_USING_NAMESPACE, name);
}

void p1_case(expptr case_, expptr lower, expptr upper)
{
    p1put(P1_CASE);
    p1_expr(case_);
    p1_expr(lower);
...
void p1puts (int type, const char *str)
//int type;
//char *str;
{
...
static void p1putd (int type, long value)
//int type;
//long value;
{
...
static void p1putdd (int type, int v1, int v2)
//int type, v1, v2;
{
...
static void p1putddd (int type, int v1, int v2, int v3)
//int type, v1, v2, v3;
{
...
void p1putds(int type, int i, const char *str)
//int type;
//int i;
//char *str;
{
...
void p1putdss(int type, int i, const char *str, const char *str2)
//int type;
//int i;
//char *str;
{
...
static void p1putdds (int token, int type, int stg, char *str)
//int token, type, stg;
//char *str;
{
