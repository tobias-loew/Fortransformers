	...
	*Values = revchain(*Values);
	...
	if (type == TYCHAR)
		...
		ind_printf(0, outfile, dimp
			? "((%s *)&equiv_%d)\n\n" : "(*(%s *)&equiv_%d)\n\n",
			c_type_decl(type,0, false).c_str(), eqvmemno);
	}
 ...
 badvar:
    ...
    *Values = values = revchain(*Values);
    ...
    if (cxx) {
        ...
        write_cxx_type(outfile, info.name, false, nullptr, false);
    }
    else {
        nice_printf(outfile, "static %s ", cpp_type_decl(info.name, 0, false).c_str());
        //nice_printf(outfile, "%s%s ", sep,
        //    cpp_type_decl(arg, is_ext).c_str());
    }
    ...
    /* check if we need a lambda initialization */

    {
        auto it = lambda_init_map.find(varname);
        if(it != lambda_init_map.end() && (it->second.lambda_required || it->second.map_required)) {

            // for large arrays pack into batches of batch_size(100) to avoid stack-overflow
            static constexpr int batch_size = 100;
            bool use_internal_lambdas = it->second.items.size() > batch_size;

            std::string once_name;
            if (is_addr) {
                once_name = info.addr->user.Charp;
            }
            else {
                once_name = info.name->cvarname;
            }


            if (use_internal_lambdas) {
                nice_printf(outfile, ";\n");
                nice_printf(outfile, "static std::once_flag _%s__once_flag;\n", once_name.c_str());

                nice_printf(outfile, "std::call_once(_%s__once_flag, \n", once_name.c_str());
                ...
                for (auto&& item : it->second.items) {
                    if(use_internal_lambdas && counter % batch_size == 0){
                        nice_printf(outfile, "[&](){ /*batch-start*/\n");
                    }
                    ...
                    nice_printf(outfile, "(");
                    if (item.index_expr) {
                        expr_out(outfile, item.index_expr);

                    }
                    else {
                        nice_printf(outfile, "%d", item.index);
                    }
                    nice_printf(outfile, ") = ");

                    if (item.data_expr) {
                        expr_out(outfile, item.data_expr);
                    }
                    else {
                        nice_printf(outfile, "%s", item.data.c_str());
                    }
...
//int type;
{
    ...
    //while (fgets(&line[0], line.size(), infile) == NULL) {
...
#define iswhite(x) (isspace (x) || (x) == ',')
    ...
    while (*pointer) {
        ...
        *end_ptr = '\0';
        ...
        if (type == TY_DATA_STRING && index >= 3) {
            data_string.push_back((char)strtol(pointer,nullptr, 16));
    ...
    if (type == TY_DATA_STRING) {
        prev_val->nextp = mkchain((char*)mkstrcon(data_string.size(), data_string.c_str()), CHNULL);
        prev_val = prev_val->nextp;
    }
...
/* Fill with zeros.  The structure shorthand works because the compiler
   ...
   */
	...
	if (!cxx && index < main_index)
		overlapping();
	else switch (type) {
	    case TYCHAR:
            { int this_char;

            if ((int)((chainp)values->datap)->nextp->datap == TY_DATA_STRING) {
                Constp c1 = (Constp)((((((chainp)values->datap)->nextp)->nextp)->nextp)->nextp)->datap;
                nice_printf(outfile, "\"");
                nice_printf(outfile, c1->Const.ccp);
                nice_printf(outfile, "\"");
                int i = 42;
            }
            else {
 ...
 //Namep *info;
{
 ...
 static chainp
do_string(FILEP outfile, chainp v, ftnint *nloc)
 //FILEP outfile;
 //register chainp v;
 //ftnint *nloc;
{
 ...
 static chainp
Ado_string(FILEP outfile, chainp v, ftnint *nloc)
 //FILEP outfile;
 //register chainp v;
 //ftnint *nloc;
{
