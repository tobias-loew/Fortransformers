...
static int needwkey;
bool seen_type_dcolon = false;
bool is_data_statement = false;
...
extern int is_free_format;
static char **linestart;
static bool free_format_current_is_continuation = false;
static bool free_format_previous_is_continuation = false;
//static bool free_format_current_is_continuation_in_string = false;
//static bool free_format_previous_is_continuation_in_string = false;
LOCAL int ncont;
...
// prefixes have to be AFTER extensions
LOCAL struct Keylist  keys[ ] =
{
    ...
    { "c_bool", SC_BOOL },
    { "c_char",  SC_CHAR },
    { "c_double_complex",  SC_DOUBLE_COMPLEX },
    { "c_double",  SC_DOUBLE },
    { "c_float_complex",  SC_FLOAT_COMPLEX },
    { "c_float128_complex",  SC_FLOAT128_COMPLEX },
    { "c_float128",  SC_FLOAT128 },
    { "c_float",  SC_FLOAT },
    { "c_int_fast128_t",  SC_INT_FAST128_T },
    { "c_int_fast16_t",  SC_INT_FAST16_T },
    { "c_int_fast32_t",  SC_INT_FAST32_T },
    { "c_int_fast64_t",  SC_INT_FAST64_T },
    { "c_int_fast8_t",  SC_INT_FAST8_T },
    { "c_int_least128_t",  SC_INT_LEAST128_T },
    { "c_int_least16_t",  SC_INT_LEAST16_T },
    { "c_int_least32_t",  SC_INT_LEAST32_T },
    { "c_int_least64_t",  SC_INT_LEAST64_T },
    { "c_int_least8_t",  SC_INT_LEAST8_T },
    { "c_int128_t",  SC_INT128_T },
    { "c_int16_t",  SC_INT16_T },
    { "c_int32_t",  SC_INT32_T },
    { "c_int64_t",  SC_INT64_T },
    { "c_int8_t",  SC_INT8_T },
    { "c_intmax_t",  SC_INTMAX_T },
    { "c_intptr_t",  SC_INTPTR_T },
    { "c_int",  SC_INT },
    { "c_long_double_complex",  SC_LONG_DOUBLE_COMPLEX },
    { "c_long_double",  SC_LONG_DOUBLE },
    { "c_long_long",  SC_LONG_LONG },
    { "c_long",  SC_LONG },
    { "c_ptrdiff_t",  SC_PTRDIFF_T },
    { "c_short",  SC_SHORT },
    { "c_signed_char",  SC_SIGNED_CHAR },
    { "c_size_t",  SC_SIZE_T },
    { "data",  SDATA  },
	...
	{ "doconcurrent", SDOCONCURRENT, YES  },
    {"elemental",  SELEMENTAL, YES},
    {"elseif",  SELSEIF, YES},
    { "else",  SELSE, YES  },
	{ "endfile",  SENDFILE  },
	{ "endif",  SENDIF, YES  },
    { "enddo", SENDDO, YES},
    {"endfunction", SENDFUNCTION, YES},
    {"endinterface",  SENDINTERFACE, YES},
    {"endsubroutine", SENDSUBROUTINE, YES},
    {"endmodule", SENDMODULE, YES},
    {"endprogram", SENDPROGRAM, YES},
    {"endselect", SENDSELECT, YES},
    {"endtype", SENDTYPE, YES},
    { "end",  SEND  },
	{ "entry",  SENTRY, YES  },
	{ "equivalence",  SEQUIV  },
    {"exit",  SEXIT},
...
LOCAL void analyz(), crunch();
static void store_comment(char *str);
LOCAL int getcds(), getkwd(), gettok();
...
//char *name;
{
    if(from_parse && cxx)
    {
        ...
        // check for ignores
        {
            ...
            auto res = std::find(std::begin(mapped_includes), std::end(mapped_includes), n);
            if(res != std::end(mapped_includes))
            {
                seen_includes[res - std::begin(mapped_includes)] = true;
                return;
            }
        }


        add_global_include(name, false);
    }
    else    
    {
        FILEP fp;
...
/* This loop merges all continuations into one long statement, AND puts the next
 ...
 static void
bang(char *a, char *b, char *c, char *d, char *e)		/* save ! comments */
 //char *a, *b, *c;
 //register char *d, *e;
{
...
//register char *b;
{
    ...
    //register char *bend;
//    int speclin;		
                        /* Special line - true when the line is allowed
                        to have more than 66 characters (e.g. the
                        "&" shorthand for continuation, use of a "\t"
                        to skip part of the label columns) */
    ...
    std::string line; // the whole line/card
...
top:
   ...
   // atend = aend;
    ...
    /* Handle the Comment cards (a '!' in the first column). */
    ...
    else if (c != EOF)
    {
        ...
        char* end;
        nxtstno = 0;
        int nxtstno_test = strtol(start, &end, 10);
        if (end != start) {
            nxtstno = nxtstno_test;
            line = line.substr(end - start);
        }
        free_format_previous_is_continuation = free_format_current_is_continuation;
        //free_format_previous_is_continuation_in_string = free_format_current_is_continuation_in_string;
  ...
  //          errstr("empty line", "");
        ...
        if(free_format_previous_is_continuation && line.front() == '&')
        { 
            line = line.substr(1);
        }


        free_format_current_is_continuation = false;
        {
            auto pos_amp = line.find('&');
            if (pos_amp != std::string::npos) {
                std::string rest = line.substr(pos_amp + 1);
                boost::algorithm::trim(rest);
                if (rest.empty()|| rest[0] == '!') {
                    free_format_current_is_continuation = true;
                    line.erase(pos_amp, 1);
                }
    ...
    if(free_format_previous_is_continuation)
    //if (!isspace(a[5]) && a[5] != '0') 
    {
        //if (!amp)
        //    for (p = a; p < aend;)
        ...
        if (nocont) {
            lineno = thislin;
            errstr("illegal continuation card (starts \"%.6s\")", "");
        }
        //else if (!amp && strncmp(a, "     ", 5)) {
        //    lineno = thislin;
        //    errstr("labeled continuation line (starts \"%.6s\")", a);
...
LOCAL int
getcd(char *b, int nocont) {
    return is_free_format
        ? getcd_free(b, nocont)
        : getcd_fixed(b, nocont)
        ;
}
		...
		if(*i=='\'' ||  *i=='"')
		{
			...
			*j = MYQUOTE; /* special marker */
			for(;;)
			{
				...
				else if(*i=='\\' && i<lastch && use_bs && !is_include) {
					++i;
 ...
 hol_overflow:
			...
			else if(parlev == 0)
            {
                if(*i == '=')
                {
                    ...
                    {
                        // check if "::" is inside parensthesis
                        auto left = s.substr(0, pos);
                        auto lpars = std::count(left.begin(), left.end(), '(');
                        auto rpars = std::count(left.begin(), left.end(), ')');
...
copychar:		/*not a string or space -- copy, shifting case if necessary */
    ...
    if(nextch <= lastch)
    {
        ...
        is_data_statement = s.find("data") == 0;
    }
}
...
int havdot, havexp, havdbl;
    ...
    if(needkwd || (needwkey_if_seen_type_dcolon && seen_type_dcolon)) {
		...
		if(ch == pp->punchar) {
			...
			if (++nextch <= lastch)
                ...
                case '/':
                    ...
                    else if (is_data_statement && new_dcl && parlev == 0)
                    {
                        val = SSLASHD;
                    }
					return val;
 ...
 extchk:
	...
	if( isalpha_(ch) )
	{
		...
		*p = 0;
        ...
        if(toklen == 7 && eqn(7, token, "reshape")
            &&
            nextch<lastch && nextch[0] == '('
            )
        {
        ...
        if(toklen == 6 && eqn(6, token, "source")
            &&
            nextch<lastch && nextch[0] == '='
            )
        {
        ...
        //if(toklen == 9 && eqn(9, token, "elemental")
        //    &&
        //    nextch<lastch // && nextch[0] == '='
        //    )
        //{
