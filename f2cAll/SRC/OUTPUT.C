...
#define FALSE 0
#endif
extern bool allocating;

struct disable_allocating_t{
    bool allocating_save;

    disable_allocating_t()
        :allocating_save(allocating)
    {
        allocating = false;
    }
...
/* This isn't quite right -- it doesn't handle arrays, for instance */
    ...
    /* OPMEMBER 62 */{BINARY_OP,   15, "%l.%r"},
    /* OPCALL_MEMBER 63 */{BINARY_OP,   16, SPECIAL_FMT},
    /* OP_OPT_SOURCE 64 */{UNARY_OP,   0, SPECIAL_FMT},
    /* OP_OPT_STAT 65 */{UNARY_OP,   0, SPECIAL_FMT},
    /* OPCALL_MEMFUNC 66 */{BINARY_OP,   16, SPECIAL_FMT},
    /* OPINIT 67 */{ BINARY_OP, 16, SPECIAL_FMT },
    /* OPSTRCAT 68 */{ BINARY_OP,  0, "ft::strcat (%l, %r)" },
    /* OPMEMBER_OF_ALLOCATABLE 69 */{BINARY_OP,   15, "%l->%r"},
...
extern bool global_force_upper_case;
...
static void output_prim(FILE *fp, struct Primblock *primp);
static void output_unary(FILE *fpfp, struct Exprblock *e);
static void output_binary(FILE *fp, struct Exprblock *e);
static void output_arg_list(FILE *fp, struct Listblock *listp);
static void output_list(FILE *fp, struct Listblock *listp);
static void output_literal(FILE *fp, int memno, Constp cp);
static void output_speciallist(FILE *fp, struct Listblock *listp);
...
//expptr e;
{
    ...
    end_out:		frexpr(e); //TODO
        return;

    case TSPECIALLIST:
        output_speciallist(fp, &(e->listblock));
        frexpr(e);
        return;
...
/* Check for array subscripts */
 ...
 static int
samefpconst(Constp c1, Constp c2, int n)
 ...
 //register Constp cp;
{
	...
	case TYLOGICAL2:
	case TYLOGICAL:
	    nice_printf (fp, "%s", cp->Const.ci ? "true" : "false");
	    break;
	case TYCHAR: {
    ...
    case TSPECIALLIST:
        {
            ...
            if(allocating)
            {
                if(srng)
                {
                    nice_printf(fp, "ft::allocating_srng<");
                }
                else
                {
                    nice_printf(fp, "ft::allocating_rng(");
                }
            }
            else
            {

                if(count_chain(ep->listblock.listp) >= 3) {
                    nice_printf(fp, "ft::stride(");
                } else {
                    if(srng) {
                        nice_printf(fp, "ft::srng<");
                    } else {
                        nice_printf(fp, "ft::rng(");
                    }
                }
            }

            {
                disable_allocating_t disable_allocating;

                bool print_comma = false;
                int i = 0;
                int max_i = -1;
                for(arglist = ep->listblock.listp;;++i) {
                    if(!(expptr)arglist->datap){
                        holdup += 
                            i==0 ? "ft::rng_first, " :
                            i == 1 ? "ft::rng_last, "
                            : "ft::nn, ";
...
//struct Addrblock *addrp;
{
    ...
    case UNAM_REFCXX:
        if(addrp->memoffset)
        {
            nice_printf(fp, "%s(", addrp->user.name->cvarname);
            out_args(fp, addrp->memoffset);
            nice_printf(fp, ")");
        }
        else
...
//struct Primblock *primp;
{
    ...
    if(primp->fcharp != (expptr)NULL ||
        primp->lcharp != (expptr)NULL ||
        primp->scharp != (expptr)NULL
        )
    {
...
static void output_unary (FILE *fp, struct Exprblock *e)
//FILE *fp;
//struct Exprblock *e;
{
    if (e == NULL)
	...
	case OPDABS:
	    ...
	    break;
    case OP_OPT_SOURCE:
        nice_printf(fp, "ft::allocate_source(");
        output_binary(fp, e);
        nice_printf(fp, ")");
        break;
    case OP_OPT_STAT:
        nice_printf(fp, "ft::allocate_stat(");
        output_binary(fp, e);
        nice_printf(fp, ")");
        break;
    case OPCALL_MEMBER:
			...
			case TADDR:
				switch(lp->addrblock.vstg) {
					...
					if (halign && e->vtype != TYCHAR) {
						nice_printf(fp, "*(%s *)",
							c_type_decl(e->vtype, 0, false).c_str());
						expr_out(fp, lp);
						return 1;
						}
...
/* Now we can actually start to write out the function invocation */
    ...
    if(op == OPCALL_MEMFUNC)
    {
        nice_printf(outfile, "mf::");
    }
...
/* Now run through the named arguments */
    ...
    if(allocating)
    {
        nice_printf(outfile, "ft::allocating_rng(");
    }
...

    {
        ...
        if (q->tag == TADDR) {
            if (q->addrblock.vtype != TY_DERIVED && 
                q->addrblock.vtype != TYINTPTR &&
                q->addrblock.vtype > TYERROR) {
                /* I/O block */
            ...
            if (!byvalue && q->addrblock.isarray
                ...
                if (Ac && narg < at->dnargs
                    ...
                    && t > TYADDR && t < TYSUBR)
                    nice_printf(outfile, "(%s*)", typename_[t]);
...
static void output_speciallist(FILE *fp, struct Listblock *listp)
{
    ...
    switch(listp->vtype)
    {
    case VTSPECIALLIST_CONST_PARAM_INIT_BRACE:
    {
        int did_one = 0;
        chainp elts;
    ...
    break;
    case VTSPECIALLIST_IMPLIED_DO:
        {
            nice_printf(fp, "PP_IMPLIED_DO(");
            if(listp)
            {
                //global_force_upper_case = true;
                for(elts = listp->listp; elts; elts = elts->nextp) {
                    if(elts->datap) {
                        if(did_one)
                            nice_printf(fp, ", ");
    ...
    case VTSPECIALLIST_ARGS_LIST:
    case VTSPECIALLIST_CONST_PARAM_INIT:
    {
            if(listp)
                for(elts = listp->listp; elts; elts = elts->nextp) {
                    if(elts->datap) {
...
//FILE *outfile;
{
    assert(loops_count > 0 && loops_count < 100);
    for(int i = 0; i <loops_count; ++i)
    {
        prev_tab (outfile);
        nice_printf (outfile, "}\n");
    }
    if(needs_breakout_for)
    {
        margin_printf(outfile, "breakout_for:\n");
    }
} /* out_end_for */
...
void out_case(FILE *outfile, expptr case_, expptr lower, expptr upper)
{
    if(!case_)
    {
        if(!lower && !upper)
        {
            nice_printf(outfile, "default:\n");
        }
        else if(!lower)
        {
            nice_printf(outfile, "cases ... : ");
            expr_out(outfile, upper);
            nice_printf(outfile, ":\n");
        }
        else if(!upper)
        {
            nice_printf(outfile, "cases ");
            expr_out(outfile, lower);
            nice_printf(outfile, " : ... :\n");
        }
        else
        {
    ...
    {
        nice_printf(outfile, "case ");
        expr_out(outfile, case_);
        nice_printf(outfile, ":\n");
    }
} /* out_case */
