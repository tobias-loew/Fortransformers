...
FILEP initfile;
FILEP blkdfile;
lambda_init_map_t lambda_init_map;
...
char *infname;
int needkwd;
int needwkey_if_seen_type_dcolon;
int trykwd;
...
#define CXX_INT_TYPE "int"
//#define CXX_INT_TYPE "long"
...
char *protorettypes[] = {
	"?", "??", "ft::integer1", "ft::shortint", CXX_INT_TYPE, //  "ft::integer",
#ifdef TYQUAD
	"ft::longint",
#endif
	"float", "double",
	"dump::C_f", "dump::Z_f", "ft::logical1", "ft::shortlogical", "ft::logical", "dump::H_f", "void"
	};
...
int init_ac[NTYPES +1] = { 0,0,0,0,0,0,0,
#ifdef TYQUAD
			  0,
#endif
			  1, 1, 0, 0, 0, 2};

proctype_t proctype = std::make_tuple(nullptr, "", TYUNKNOWN);	/* Type of return value in this procedure */
char *procname;
...
bool seen_includes[includes__SIZE] = {};
...
struct Hashentry *type_hashtab;	/* Hash table */
struct Hashentry *type_lasthash;
int type_hash_counter;

struct Hashentry *member_hashtab;	/* Hash table */
struct Hashentry *member_lasthash;
int member_hash_counter;


std::vector<std::string> global_includes;
std::vector<std::string> proto_includes;
...
void fileinit()
{
	...
	ctls = new Ctlframe[maxctl+1];
	extsymtab = ALLOCN(maxext, Extsym);
	eqvclass = ALLOCN(maxequiv, Equivblock);
    hashtab = ALLOCN(maxhash, Hashentry);
    type_hashtab = ALLOCN(maxhash, Hashentry);
    member_hashtab = ALLOCN(maxhash, Hashentry);
    ...
    type_lasthash = type_hashtab + maxhash;
    member_lasthash = member_hashtab + maxhash;
...
void type_hashclear()	/* clear hash table */
{
    register struct Hashentry *hp;
    register Namep p;
    ...
    for(hp = type_hashtab; hp < type_lasthash; ++hp)
        if(p = hp->varp)
        {
            // ignore unfreed memory...
...
void member_hashclear()	/* clear hash table */
{
    register struct Hashentry *hp;
    register Namep p;
    ...
    for(hp = member_hashtab; hp < member_lasthash; ++hp)
        if(p = hp->varp)
        {
            // ignore unfreed memory...
...
void procinit()
{
    ...
    for(auto&& seen_include : seen_includes)
    {
        seen_include = false;
    }


	std::get<2>(proctype) = TYUNKNOWN;
	procname = "MAIN_";
	procclass = CLUNKNOWN;
	nentry = 0;
