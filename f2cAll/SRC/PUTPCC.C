...
LOCAL Addrp intdouble();
LOCAL Addrp putch1(expptr p);
LOCAL Addrp putcx1(expptr p);
LOCAL Addrp putcxeq(expptr p);
LOCAL void putct1(expptr q, Addrp length_var, Addrp string_var, int *ip);

expptr putcxop();
LOCAL expptr putcall(expptr p0, Addrp *temp);
LOCAL expptr putmnmx(expptr p);

LOCAL expptr putcheq(expptr p);
LOCAL expptr putcat(expptr lhs0, expptr rhs);
LOCAL expptr putaddr(expptr p0);
LOCAL expptr putop(expptr p);
LOCAL expptr putpower(expptr p);
LOCAL expptr putchcmp(expptr p);
LOCAL expptr putcxcmp(expptr p);
ftnint lencat();
LOCAL expptr putmember(expptr p);
LOCAL expptr putspeciallist(expptr p);
    ...
    case OPCONV:
            ...
            (INT(pt) && (ONEOF(lt, MSKINT | MSKADDR | MSKCHAR | M(TYSUBR))))))
        {
            ...
            if (lt == TYCHAR && lp->tag == TEXPR &&
                ...
                || lp->exprblock.opcode == OPINIT
                || lp->exprblock.opcode == OPCALL_MEMBER
                || lp->exprblock.opcode == OPCALL_MEMFUNC
                || lp->exprblock.opcode == OP_OPT_SOURCE
                || lp->exprblock.opcode == OP_OPT_STAT
                )
            {
...
LOCAL expptr putspeciallist(expptr p)
{
    //for(auto cp = p->listblock.listp; cp; cp = cp->nextp)
    //{
    //    q = (expptr)cp->datap;
    //}
...
//register expptr p;
{
	...
	case TADDR:
		...
		ts = tskludge = 0;
		if (q = resp->memoffset) {
			if (resp->uname_tag == UNAM_REF || resp->uname_tag == UNAM_REFCXX) {
...
/* If s = t // u, don't bother copying the result, write it directly into
   this buffer */
	...
	else
    {
        ...
        {
            p = putop(mkexpr(OPASSIGN, lp, rp));
//            p = putx(call2(TYSUBR, "ft::string_copy", lp, rp));
        }
        else
        {
...
//register expptr p;
{
    ...
    if (!cxx && ISONE(lp->headblock.vleng) && ISONE(rp->headblock.vleng)) {
        lp = mkexpr(OPCONV, lp, ENULL);
        rp = mkexpr(OPCONV, rp, ENULL);
        lp->headblock.vtype = rp->headblock.vtype = TYCHAR;
    }
        ...
        case OPEQ:
        {
            return call2(TYINT, "ft::string_equal", lp, rp);
        }
        break;
        case OPNE:
        {
            return call2(TYINT, "ft::string_not_equal", lp, rp);
        }
        break;

        default: 
            {
            lp = call2(TYINT, "ft::string_cmp", lp, rp);
            rp = ICON(0);
            }
            break;
        }
 ...
 //expptr p0;
{
	...
	if (p->isarray && p->memoffset)
		if (p->uname_tag == UNAM_REF || p->uname_tag == UNAM_REFCXX) {
 ...
 LOCAL expptr
addrfix(expptr e)		/* fudge character string length if it's a TADDR */
 //expptr e;
{
 ...
 LOCAL int
typekludge(int ccall, expptr q, Atype *at, int j)
 //int ccall;
 //register expptr q;
 //Atype *at;
 ...
 //char *fname;
{
	...
	static int *init_ap[NTYPES +1] = {0,0,0,0,0,0,0,
#ifdef TYQUAD
							0,
#endif
 ...
 void
saveargtypes(Exprp p)		/* for writing prototypes */
 //register Exprp p;
{
	Addrp a;
	Argtypes **at0, **at1;
	...
	switch(a->vstg) {
		case STGEXT:
			...
			break;
		case STGARG:
			if (a->uname_tag != UNAM_NAME && (!cxx || a->uname_tag != UNAM_REFCXX))
				goto bug;
			np = a->user.name;
			at0 = at1 = &np->arginfo;
			fname = np->fvarname;
...
/* If the routine is a COMPLEX function ... */
 ...
 //   p->leftp = putx(fixtype(putaddr(p->leftp)));
    p->leftp = putx(fixtype(putx(p->leftp)));
 ...
 //expptr p;
{
	...
	if(!cxx && !ISLOGICAL((k = (p = fixtype(p))->headblock.vtype)))
		{
		if(k != TYERROR)
