...
flag checksubs = NO;
flag r8flag = NO;
flag dolimitconstflag = NO;
flag no_var_pooling = NO;
flag unused_vars = NO;
flag symbolic_params = NO;
flag cxx = NO; /* use modern c++ (implies a lot) */
flag srng = NO; /* use static rng */
flag use_member_op = YES; /* use member_op () for strust member access */
flag use_bs = YES;
flag keepsubs = NO;
#ifdef TYQUAD
flag use_tyquad = YES;
...
static arg_info table[] = {
    ...
    f2c_entry ("dc", P_NO_ARGS, P_INT, &dolimitconstflag, YES),
    f2c_entry ("np", P_NO_ARGS, P_INT, &no_var_pooling, YES),
    f2c_entry ("uv", P_NO_ARGS, P_INT, &unused_vars, YES),
    f2c_entry ("sp", P_NO_ARGS, P_INT, &symbolic_params, YES),
    f2c_entry ("cxx", P_NO_ARGS, P_INT, &cxx, YES),
    f2c_entry("f77", P_NO_ARGS, P_INT, &is_free_format, NO),
    f2c_entry("f90", P_NO_ARGS, P_INT, &is_free_format, YES),
...
extern char *h_functions;	/* "c_functions"	*/
extern std::string coutput;		/* "c_output"		*/
...
extern std::string proto_fname;	/* "proto_file"		*/
FILE *protofile;
FILE *protofile_final;

extern void set_tmp_names(), sigcatch();
//extern char *c_name();
...
/* Adjust the global flags according to the command line parameters */
    ...
    inqmask = M(TYLONG);
 checklong:
    protorettypes[TYLOGICAL] = typename_[TYLOGICAL] = "shortlogical";
...
\n\n#ifndef F2C_proc_par_types\n");
            ...
            nice_printf(outfile, "typedef %s (*%s)%s;\n",
                    c_type_decl(i,1, false).c_str(), s, p);
            }
...
void fix_module_include(std::string& n)
{
    // adjust include name, e.g. remove "_module" from the following includes

    static std::vector<std::string> module_names = {
        //"calculation_module.h",
        //"output_module.h",
    };

    // check for module_names
    {
        std::string n2 = n;
        std::transform(n2.begin(), n2.end(), n2.begin(), ::tolower);
        if(std::find(module_names.begin(), module_names.end(), n2) != module_names.end())
        {
            return;
        }
    }

    auto pos = n.find("_module.h");
...
//char **argv;
{
    ...
    protofile = opf(h_functions, textwrite);
    pass1_file=opf(p1_file, binwrite);
    initkey();
    if (file_name && *file_name) {
        if (is_free_format == UNDECIDED) {
            is_free_format = NO;
            auto extension = boost::to_lower_copy(get_extension(file_name));
            if (extension == "f90") {
                is_free_format = YES;
            }
        }
        ...
        if (debugflag != 1) {
            ...
            cpp_output += "pp";
            if (Castargs1 >= 2)
                proto_fname = c_name(file_name,"h");
            }
        cdfilename = cpp_output.c_str();
        if (skipC)
            coutput.empty();
        else if (!(c_output = fopen(cpp_output.c_str(), textwrite))) {
            //file_name = coutput.c_str();
            fatalstr("can't open %.86s", coutput.c_str());
            coutput.empty();	/* don't delete read-only .c file */
            }

        if (Castargs1 >= 2
        && !(protofile_final = fopen(proto_fname.c_str(), textwrite)))
        ...
        if (Castargs1 >= 2) {
            protofile = stdout;
            protofile_final = stdout;
            if (!skipC)
                printf("#ifdef P_R_O_T_O_T_Y_P_E_S\n");
            }
        }

    nice_printf(protofile_final, "#pragma once\n\n");
    nice_printf(protofile_final, "#include \"f2cpp.h\"\n\n");
    nice_printf(protofile_final, "#include \"f2cpp_typedefs.h\"\n\n");

    {
        // add specific includes for specific files
        //std::string proto_name = proto_fname;
        //std::transform(proto_name.begin(), proto_name.end(), proto_name.begin(), ::tolower);
        //if(proto_name.substr(0, 8) == "src\\baut" && (proto_name.substr(11) == ".h" || (proto_name.substr(11) == "_physik.h")
        //    ))
        //{
        //    nice_printf(protofile_final, "#include \"AutoGenerated/baut");
        //    nice_printf(protofile_final, proto_name.substr(8,3).c_str());
        //    nice_printf(protofile_final, "_declaration.h\"\n\n");

        //    nice_printf(protofile_final, "using namespace baut");
        //    nice_printf(protofile_final, proto_name.substr(8, 3).c_str());
        //    nice_printf(protofile_final, "_declaration;\n\n");
    ...
    if(!proto_includes.empty())
    {
        nice_printf(protofile_final, "// includes \n");
        for(auto n : proto_includes){
            fix_module_include(n);
            nice_printf(protofile_final, "#include \"%s\"\n", n.c_str());
        }
        nice_printf(protofile_final, "\n\n");
    }

    commonprotos(protofile_final);
    if (protofile_final == stdout && !skipC)
        printf("#endif\n\n");

    fclose(protofile);
    if((protofile = fopen(h_functions, textread)) == (FILE *)NULL)
        Fatal("main - couldn't reopen h_functions");

    ffilecopy(protofile, protofile_final);
...
/*>>>'%s'<<<*/\n", cdfilename);
    ...
    if (Ansi == 2)
        nice_printf(c_output,
            "#ifdef __cplusplus\nextern \"C\" {\n#endif\n");
    nice_printf(c_output, "#include \"stdafx.h\"\n\n");
    nice_printf(c_output, "#include \"f2cpp.h\"\n");
    nice_printf(c_output, "#include \"f2cpp_externals.h\"\n");

    {
        ...
        nice_printf(c_output, "#include \"%s\"\n\n", n.c_str());
    }
    if (gflag)
        nice_printf (c_output, "#line 1 \"%s\"\n", file_name);
    ...
    {
        nice_printf(c_output, "// includes \n");
        for(auto n : global_includes){
            fix_module_include(n);
            nice_printf(c_output, "#include \"%s\"\n", n.c_str());
        }
        nice_printf(c_output, "\n\n");
    ...
    if (*main_alias) {
        if (cxx) {
            ...
            nice_printf(c_output, "int %s () { return main();}\n",
                main_alias);
        }
        else
        {
